...
#define IDM_UM_COPY_ID				WM_USER+101
#define	IDM_UM_SET_CHAT				WM_USER+102							//设置聊天
#define	IDM_UM_CANECL_CHAT			WM_USER+103							//取消聊天
#define	IDM_UM_ALLOW_LOOKON			WM_USER+104							//旁观控制
#define	IDM_UM_ENJOIN_LOOKON		WM_USER+105							//旁观控制
#define	IDM_UM_KICK_TABLE_USER		WM_USER+106							//踢走用户


///////////////////////////////////////////////////////////////////////////
//边框资源
static CSkinImage					m_ImageFrameRB;						//边框图片
static CSkinImage					m_ImageFrameRM;						//边框图片
static CSkinImage					m_ImageFrameRT;						//边框图片
...
CGameFrameControl::CGameFrameControl(CGameFrameDlg * pGameFrameDlg) : CDialog(IDD_FRAME_CONTROL), m_pGameFrameDlg(pGameFrameDlg)
{
	...
	m_ImageFrameRB.SetLoadInfo(IDB_FRAME_RB,hInstance,false);
	m_ImageFrameRM.SetLoadInfo(IDB_FRAME_RM,hInstance,false);
	m_ImageFrameRT.SetLoadInfo(IDB_FRAME_RT,hInstance,false);
...
BOOL CGameFrameControl::OnInitDialog()
{
	...
	//广告控件
	m_BrowerAD.Create(NULL,NULL,WS_CHILD,CRect(0,0,0,0),this,100,NULL);
	m_BrowerAD.Navigate(TEXT("http://www.52e8.com/AD/GameFrameAD.asp"),NULL,NULL,NULL,NULL);
	...
	case IDM_UM_COPY_ID:	//拷贝 I D
		{
			...
			sprintf(szString,TEXT("%ld"),m_pUserData->dwGameID);
			SetClipboardData(CF_TEXT,hData);
			GlobalUnlock(hData);
			CloseClipboard();
			break;
...
void CGameFrameControl::OnBnClickedRule()
{
	TCHAR szRuleUrl[128];
	//CString sz_temp;
	//sz_temp.Format("%s",m_UserListView.m_wKindID);
	//AfxMessageBox("ok");
	//if(sz_temp=="3310")
	//{
	//  AfxMessageBox("ok!!!");
	//}
	_snprintf(szRuleUrl,sizeof(szRuleUrl),TEXT("http://www.52e8.com/GameRule.asp?KindID=%ld"),m_UserListView.m_wKindID);
...
void CGameFrameControl::OnBnClickedLookon()
{
	...
	ShellExecute(NULL,TEXT("open"),TEXT("http://www.52e8.com/Invite.asp"),NULL,NULL,SW_SHOWDEFAULT);
...
void CGameFrameControl::DrawLeftViewFrame(CDC * pDC)
{
	...
	pDC->FillSolidRect(rcButton,RGB(244,244,244));
	...
	//边框资源
	CImageHandle ImageHandleFrameLB(&m_ImageFrameRB);
	CImageHandle ImageHandleFrameLM(&m_ImageFrameRM);
	CImageHandle ImageHandleFrameLT(&m_ImageFrameRT);
	CImageHandle ImageHandleFrameBM(&m_ImageFrameBM);
	CImageHandle ImageHandleFrameBR(&m_ImageFrameBR);
	...
	//绘画边框
	m_ImageFrameRT.BitBlt(pDC->GetSafeHdc(),rcClient.right-m_ImageFrameRT.GetWidth(),0);
	for (int nYPos=m_ImageFrameRT.GetHeight(); nYPos<rcClient.bottom-m_ImageFrameRB.GetHeight(); nYPos+=m_ImageFrameRM.GetHeight())
		m_ImageFrameRM.BitBlt(pDC->GetSafeHdc(),rcClient.right-m_ImageFrameRT.GetWidth(),nYPos);
	m_ImageFrameRB.BitBlt(pDC->GetSafeHdc(),rcClient.right-m_ImageFrameRT.GetWidth(),rcClient.bottom-m_ImageFrameRB.GetHeight());

	for (int nXPos=0; nXPos<rcClient.right; nXPos+=m_ImageFrameBM.GetWidth())
		m_ImageFrameBM.BitBlt(pDC->GetSafeHdc(),nXPos,rcClient.bottom-m_ImageFrameBR.GetHeight());
	m_ImageFrameBR.BitBlt(pDC->GetSafeHdc(),rcClient.right-m_ImageFrameBR.GetWidth(),rcClient.bottom-m_ImageFrameBR.GetHeight());
...
void CGameFrameControl::RectifyControl(int nWidth, int nHeight)
{
	...
	//变量定义
	const int nComminutePos=nHeight*5/9;
	...
	DeferWindowPos(hDwp, m_PropertyBar, NULL, m_ImageInfoList.nLBorder-2, nComminutePos-m_PropertyBar.GetHeight(), 
		nWidth-m_ImageInfoList.nLBorder-m_ImageInfoList.nRBorder+5, m_PropertyBar.GetHeight(), uFlags); 

	//列表区域
	DeferWindowPos(hDwp,m_UserListView,NULL,m_ImageInfoList.nLBorder,m_ImageInfoList.nTBorder,
		nWidth-m_ImageInfoList.nLBorder-m_ImageInfoList.nRBorder,
		nComminutePos-m_ImageInfoList.nTBorder-m_ImageInfoList.nBBorder-m_PropertyBar.GetHeight(),uFlags);
	...
	//聊天区域
	DeferWindowPos(hDwp,m_ChatMessage,NULL,m_ImageInfoChat.nLBorder,nComminutePos+m_ImageInfoChat.nTBorder+37,nWidth-m_ImageInfoChat.nRBorder-m_ImageInfoChat.nLBorder,
		nHeight-nComminutePos-m_ImageInfoChat.nTBorder-m_ImageInfoChat.nBBorder-37,uFlags);
	...
	{
		DeferWindowPos(hDwp,pButtonArray[i]->m_hWnd,NULL,m_ImageInfoList.nLBorder+(rcButton.Width()+5)*i,nHeight-49,rcButton.Width(),rcButton.Height(),uFlags|SWP_NOSIZE);
	}
	...
	DeferWindowPos(hDwp,m_ChatObject,NULL,8,nHeight-26,70,200,uFlags);
	DeferWindowPos(hDwp,m_ChatInput,NULL,81,nHeight-26,nWidth-rcButton.Width()-93,19,uFlags);

	//功能按钮
	CRect rcOption,rcRule,rcLookOn;
	m_btGameOption.GetWindowRect(&rcOption);
	m_btGameRule.GetWindowRect(&rcRule);
	m_btLookOn.GetWindowRect(&rcLookOn);
	int nBeginPos=12,nEndPos=nWidth-8;
	int nButtonSpace=(nEndPos-nBeginPos-rcOption.Width()-rcRule.Width()-rcLookOn.Width())/2,nYPosButton=nComminutePos+5;
	/*DeferWindowPos(hDwp,m_btSound,NULL,nBeginPos,nYPosButton,rcButton.Width(),rcButton.Height(),uFlags|SWP_NOSIZE);*/
	DeferWindowPos(hDwp,m_btGameOption,NULL,nBeginPos,nYPosButton+4,rcButton.Width(),rcButton.Height(),uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btGameRule,NULL,nBeginPos+nButtonSpace+rcOption.Width(),nYPosButton+4,rcButton.Width(),rcButton.Height(),uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btLookOn,NULL,nBeginPos+nButtonSpace*2+rcOption.Width()+rcRule.Width(),nYPosButton+4,rcButton.Width(),rcButton.Height(),uFlags|SWP_NOSIZE);
	//DeferWindowPos(hDwp,m_btGameQuit,NULL,nBeginPos+nButtonSpace*4+rcButton.Width()*4,nYPosButton,rcButton.Width(),rcButton.Height(),uFlags|SWP_NOSIZE);
	m_btGameQuit.ShowWindow(SW_HIDE);

	//用户区域
	DeferWindowPos(hDwp,m_UserInfoView,NULL,m_ImageInfoList.nLBorder-2,0,nWidth-m_ImageInfoList.nLBorder-m_ImageInfoList.nRBorder,145,uFlags);
...
void CGameFrameControl::OnNMRclickUserList(NMHDR * pNMHDR, LRESULT * pResult)
{
	...
	if (pListNotify->iItem!=-1)
	{
		...
		//用户积分
		if (GAME_GENRE_GOLD==g_GlobalOption.m_wGameGenre)
            _snprintf(szBuffer,sizeof(szBuffer),TEXT("金币：%I64d  总局数：%ld  逃跑率：%5.2f%%"),m_pUserData->lScore,lPlayCount,dFleeRate);
		else
