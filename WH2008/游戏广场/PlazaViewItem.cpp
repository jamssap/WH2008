	...
	case SUB_GP_LOGON_SUCCESS:		//登录成功
		{
			...
			//设置按钮
			UINT uBtResurce=UserData.cbMoorMachine?IDB_BT_UNLOCK:IDB_BT_LOCK;
			CGameFrame *pGameFrame=(CGameFrame*)AfxGetMainWnd();
			pGameFrame->m_btButton6.SetButtonImage(uBtResurce,AfxGetInstanceHandle(),false);
	...
	case SUB_GP_LOGON_FINISH:		//登录完成
		{
			...
			//置能按钮
			((CGameFrame*)AfxGetMainWnd())->EnableButtons(true);
	...
	case SUB_GP_SAFE_BIND:					//绑定机器
		{
			...
			if (!pResult->cbSuccess)
			{
				...
				//设置标识
				GlobalUserData.cbMoorMachine=1;
				...
				//设置按钮
				CGameFrame *pGameFrame=(CGameFrame*)AfxGetMainWnd();
				pGameFrame->m_btButton6.SetButtonImage(IDB_BT_UNLOCK,AfxGetInstanceHandle(),false);
			}
			else
			{
				//显示消息
	...
	case SUB_GP_SAFE_UNBIND:				//绑定机器
		{
			...
			if (!pResult->cbSuccess)
			{
				...
				//设置标识
				GlobalUserData.cbMoorMachine=0;
				...
				//设置按钮
				CGameFrame *pGameFrame=(CGameFrame*)AfxGetMainWnd();
				pGameFrame->m_btButton6.SetButtonImage(IDB_BT_LOCK,AfxGetInstanceHandle(),false);
			}
			else
			{
				//显示消息
...
void CPlazaViewItem::OnCommandLogon()
{
	...
	//置能按钮
	((CGameFrame*)AfxGetMainWnd())->EnableButtons(false);
...
void CPlazaViewItem::OnCommandConnect()
{
	...
	if (RegServer.Open(HKEY_CURRENT_USER,szRegServer,KEY_READ)==ERROR_SUCCESS)
	{
		...
		if (lErrorCode==ERROR_SUCCESS) 
		{
           // szServerAddr=TEXT("122.228.217.66");
			//sprintf(szServerAddr,"%s","122.228.217.66");
			CXOREncrypt::CrevasseData(szReadData,szServerAddr,sizeof(szServerAddr));
			pszServerIP=szServerAddr;
			//AfxMessageBox(szServerAddr);
            //pszServerIP="122.228.217.66";
		}
		else
		{
	...
	//显示提示框
	if (enBindAction_NULL==m_enBindAction)
	{
...
{
	m_pHtmlBrower->Navigate("http://www.52e8.com");
}
