...
bool __cdecl CAttemperEngineSink::OnAttemperEngineStart(IUnknownEx * pIUnknownEx)
{
	...
	CSystemLog::GetInstance().Create(m_pGameServiceAttrib->wKindID,m_pGameServiceOption->wServerID,m_pGameServiceAttrib->szKindName);

	OUTLOG(TEXT("**************［%s］游戏服务启动完成****************"),m_pGameServiceAttrib->szKindName);
...
bool __cdecl CAttemperEngineSink::OnAttemperEngineStop(IUnknownEx * pIUnknownEx)
{
	...
	OUTLOG(TEXT("**************［%s］游戏服务已停止运行****************"),m_pGameServiceAttrib->szKindName);

	CSystemLog::GetInstance().Close();
...
bool __cdecl CAttemperEngineSink::OnEventDataBase(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBR_GR_BANK_RESULT:
		{
			return OnDBBankResult(dwContextID,pData,wDataSize);
		}
	}
...
bool CAttemperEngineSink::OnDBLogonSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	{
		LPCTSTR pszMessage=TEXT("请注意，这是游戏练习房间，你将得到一定数目的模拟金币，游戏成绩将不会记录到数据库！");
...
//银行结果
bool CAttemperEngineSink::OnDBBankResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	ASSERT(pBankResult->dwUserID!=0);

	IServerUserItem *pIServerUserItem=m_ServerUserManager.SearchOnLineUser(pBankResult->dwUserID);
	if (pIServerUserItem!=NULL)
	{
		//解除锁定
		pIServerUserItem->UnlockUserBank();

		//修改金币
		if (pBankResult->lErrorCode==0 && pBankResult->cbUserOnline==FALSE)
		{
			ModifyBankStorageGold(pIServerUserItem, -pBankResult->lModifyGold);
		}
	
		CMD_GF_BankResult BankResult;
		ZeroMemory(&BankResult,sizeof(BankResult));
		BankResult.lErrorCode=pBankResult->lErrorCode;
		BankResult.lResultType=pBankResult->lResultType;
		BankResult.cbGameAction=pBankResult->cbGameAction;
		strcpy(BankResult.szErrorDescribe,pBankResult->szErrorDescribe);
		SendData(pIServerUserItem,MDM_GF_BANK,SUB_GF_BANK_RESULT,&BankResult,sizeof(BankResult));
	}
	...
	case SUB_GR_USER_LOOKON_REQ:	//旁观桌子
		{
			...
			if (wSubCmdID==SUB_GR_USER_SIT_REQ)	
			{
				...
				if (m_pGameServiceAttrib->wChairCount>MAX_CHAIR_NORMAL)
				{
					...
					//搜索位置
					//if (pTableFrame->IsGameStarted() && m_pGameServiceAttrib->cbJoinInGame==TRUE)
					//{
					...
					//CString str;
					//str.Format("%d", m_pGameServiceAttrib->wChairCount);
					//AfxMessageBox(str);
...
//发送结果
void CAttemperEngineSink::SendBankResult(IServerUserItem *pIServerUserItem, LONG lResultType, LONG lErrorCode, LPCTSTR pszDescribe, BYTE cbGameAction)
{
	ASSERT(pIServerUserItem!=NULL);
	...
	CMD_GF_BankResult BankResult;
	ZeroMemory(&BankResult,sizeof(BankResult));
	BankResult.cbGameAction=cbGameAction;
	BankResult.lResultType=lResultType;
	BankResult.lErrorCode=lErrorCode;
	ASSERT(lstrlen(pszDescribe)<sizeof(BankResult.szErrorDescribe));
	strcpy(BankResult.szErrorDescribe,pszDescribe);
	SendData(pIServerUserItem,MDM_GF_BANK,SUB_GF_BANK_RESULT,&BankResult,sizeof(BankResult));
}
...
bool CAttemperEngineSink::OnEventBankDrawoutGold(const void * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	//{
	//	SendBankResult(pIServerUserItem,2,0,TEXT("您正在游戏状态中，不能进行银行操作！"),pBankGet->cbGameAction);
	//	return true;
	//}
	...
	{
		SendBankResult(pIServerUserItem,2,0,TEXT("密码有误，请查证后再次尝试操作！"),pBankGet->cbGameAction);
		return true;
	}

	if (pIServerUserItem->IsLockedUserBank())
	{
		SendBankResult(pIServerUserItem,2,0,TEXT("您上一次的银操请求尚未完成，请稍后！"),pBankGet->cbGameAction);
		return true;
	}
	...
	SendBankResult(pIServerUserItem,2,0,TEXT("金币提取成功！"),pBankGet->cbGameAction);
	...
	BankDrawoutGold.lCurrentScore = lGameGold;
	BankDrawoutGold.lCurrentBankScore = lStorageGold;
...
bool CAttemperEngineSink::OnEventBankStorage(const void * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	//判断状态
	if ((US_SIT==pServerUserData->cbUserStatus)||(US_PLAY==pServerUserData->cbUserStatus))
	{
		SendBankResult(pIServerUserItem,1,0,TEXT("您正在游戏中，暂不能进行银行存款操作，请退出游戏再尝试！"),pBankStorage->cbGameAction);
		return true;
	}
	...
	if (pIServerUserItem->IsLockedUserBank())
	{
		SendBankResult(pIServerUserItem,1,0,TEXT("您上一次的银操请求尚未完成，请稍后！"),pBankStorage->cbGameAction);
		return true;
	}
	...
	SendBankResult(pIServerUserItem,1,0,TEXT("金币存储成功！"),pBankStorage->cbGameAction);
	...
	BankStorage.lCurrentScore = lGameGold;
	BankStorage.lCurrentBankScore = lBankGold;
...
bool CAttemperEngineSink::OnEventBankTransfer(const void * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	//计算扣税
	LONGLONG lRevenue=pBankTransfer->lTransferValue * 0.01;
	...
	{
		TCHAR szMessage[255]=TEXT("");
		
		if (pITargetUserItem->IsLockedUserBank()==false)
		{
			ModifyBankStorageGold(pIServerUserItem,-pBankTransfer->lTransferValue);
			ModifyBankStorageGold(pITargetUserItem,pBankTransfer->lTransferValue-lRevenue);
			sprintf(szMessage, TEXT("您已收到［%s］金币转入，金币数目为：%I64d，扣税：%I64d金币"), 
				pServerUserData->szAccounts, pBankTransfer->lTransferValue, lRevenue);
			SendRoomMessage(pITargetUserItem,szMessage,SMT_EJECT);
		}
		else
		{
			sprintf(szMessage, TEXT("您已收到［%s］金币转入，金币数目为：%I64d，扣税：%I64d金币，重新进入房间后生效！"), 
				pServerUserData->szAccounts, pBankTransfer->lTransferValue, lRevenue);
			SendRoomMessage(pITargetUserItem,szMessage,SMT_EJECT);
			//锁住银行
			pIServerUserItem->LockUserBank();
		}
	}
	else
	{
	...
	BankTransfer.lTransferCount=pBankTransfer->lTransferValue;
	...
	BankTransfer.cbUserOneline=pIServerUserItem->IsLockedUserBank()?FALSE:TRUE;
	...
	if(m_pGameServiceOption->wServerType == GAME_GENRE_GOLD)
	{
		...
		if((pIServerUserItem->GetTableID()!=INVALID_TABLE) && (lLessScore>0) && (lLessScore>pServerUserData->UserScoreInfo.lScore))
		{
			...
			if (!(pTableFrame->IsUserPlaying(pIServerUserItem)))
			{
				...
				_snprintf(szDescribe,sizeof(szDescribe),TEXT("加入游戏至少需要 %I64d 的金币，您的金币不够，不能加入！"),lLessScore);
		...
		if((pIServerUserItem->GetTableID()!=INVALID_TABLE) && (lMaxScore>m_pGameServiceOption->lLessScore) && (lMaxScore<pServerUserData->UserScoreInfo.lScore))
		{
			...
			if (!(pTableFrame->IsUserPlaying(pIServerUserItem)))
			{
				//发送信息
				TCHAR szDescribe[256]=TEXT("");
				_snprintf(szDescribe,sizeof(szDescribe),TEXT("您的金币超过本房间的最大限制额度%I64d,请更换游戏房间!"),lMaxScore);
				SendGameMessage(pIServerUserItem,szDescribe,SMT_EJECT|SMT_INFO|SMT_CLOSE_GAME);
