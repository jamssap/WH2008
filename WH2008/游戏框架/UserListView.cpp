...
void CUserListView::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	for (int i=0;i<iCount;i++)
	{
		...
		if ( i == 0 )
		{
			...
			else if ( 0 < pIUserItem->cbMemberOrder && 10 != pIUserItem->cbMemberOrder && (DTP_USER_ACCOUNTS == m_wDataDescribe[i])) pDC->SetTextColor(RGB(255,0,0));
			else if ( 10 == pIUserItem->cbMemberOrder && (DTP_USER_ACCOUNTS == m_wDataDescribe[i])) pDC->SetTextColor(RGB(153,50,205));
		...
		{		
			...
			if (lpDrawItemStruct->itemState&ODS_FOCUS) pDC->SetTextColor(RGB(255,255,255));
			else if ( 0 < pIUserItem->cbMemberOrder && 10 != pIUserItem->cbMemberOrder && (DTP_GAME_ID == m_wDataDescribe[i])) pDC->SetTextColor(RGB(255,0,0));
			else if ( 10 == pIUserItem->cbMemberOrder && (DTP_GAME_ID == m_wDataDescribe[i])) pDC->SetTextColor(RGB(153,50,205));
			else pDC->SetTextColor(RGB(0,0,0));
...
LPCTSTR CUserListView::GetDisplayText(tagUserData * pUserData, WORD wColumnPos)
{
	...
	case DTP_INSURE_SCORE:		//存储金币
		{
			_snprintf(szBuffer,sizeof(szBuffer),TEXT("%I64d"),pUserData->lInsureScore);
			return szBuffer;
		}
	case DTP_GAME_GOLD:			//游戏金币
		{
			_snprintf(szBuffer,sizeof(szBuffer),TEXT("%I64d"),pUserData->lGameGold);
			return szBuffer;
		}
