...
void __cdecl CTableView::DrawTable(CDC * pDC, bool bHideInfo, CTableResource * pTableResource, bool bShowUser)
{
	...
	for (WORD i=0;i<m_TableState.wChairCount;i++)
	{
		...
		if ((bHideInfo==false)&&(bShowUser==true)&&(m_TableState.pIUserItem[i]!=NULL))
		{
			...
			//名字等级
			TCHAR szName[32];
			_sntprintf(szName,CountArray(szName),TEXT("%s"),pUserData->szName);
			szName[16]=0;

			//位置变量
			int nYPos=0;
			int nDrawStyte=pTableResource->m_nDrawStyle[i];
			int nDrawFormat=DT_WORDBREAK|DT_EDITCONTROL;
			int nNameLength=lstrlen(szName);
			int nNameHeight=pDC->DrawText(szName,nNameLength,&rcDrawRect,nDrawFormat|DT_CALCRECT);
			...
			//用户名字
			pDC->SetTextColor(pTableResource->m_crName);
			pDC->DrawText(szName,nNameLength,&rcDrawRect,nDrawFormat|DT_TOP);

			//游戏等级
			TCHAR szGameRank[32];
			if (CUserListView::m_GameRankManager.GetInterface()!=NULL)
			{
				LPCTSTR pszGameRank=CUserListView::m_GameRankManager->GetLevelDescribe(pUserData->lScore);
				_sntprintf(szGameRank,CountArray(szGameRank),TEXT("[%s]"),pszGameRank);
			}

			//单行高度
			nNameHeight=pDC->DrawText(TEXT("行"),2,&rcDrawRect,nDrawFormat|DT_CALCRECT);
			rcDrawRect.left=pTableResource->m_rcName[i].left;
			rcDrawRect.right=pTableResource->m_rcName[i].right;
			rcDrawRect.top=pTableResource->m_rcName[i].top/*+nYPos*/;
			rcDrawRect.bottom=pTableResource->m_rcName[i].bottom;
			rcDrawRect.top+=(2*nNameHeight);
			pDC->SetTextColor(RGB(153,204,204));
			pDC->DrawText(szGameRank,lstrlen(szGameRank),&rcDrawRect,nDrawFormat|DT_TOP|DT_LEFT);
			//pDC->DrawText(szName,nNameLength,&rcDrawRect,nDrawFormat|DT_CALCRECT);
			//pDC->DrawText(szName,nNameLength,&rcDrawRect,nDrawFormat);
		}
	...
	if ((m_bLeftMouseDown==true)&&(m_bRightMouseDown==false))
	{
		...
		if ((wTableID==m_wDownTableID)&&(wChairID==m_wDownChairID))
		{
			...
			if (m_wChairCount > MAX_CHAIR_NORMAL)
			{
				...
				{
					if (GetUserInfo(wTableID,i)==NULL)
					{
						wChairID=i;
						break;
					}
