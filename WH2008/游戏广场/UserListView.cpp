...
CGameRankManagerHelper	CUserListView::m_GameRankManager;					//游戏等级
...
void CUserListView::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	for (int i=0;i<iCount;i++)
	{
		...
		if ( i == 0 )
		{
			...
			else if ( 0 < pIUserItem->GetUserData()->cbMemberOrder && 10 != pIUserItem->GetUserData()->cbMemberOrder && DTP_USER_ACCOUNTS== m_wDataDescribe[i]) pDC->SetTextColor(RGB(255,0,0));
			else if(10 == pIUserItem->GetUserData()->cbMemberOrder && DTP_USER_ACCOUNTS== m_wDataDescribe[i]) pDC->SetTextColor(RGB(153,50,205));
		...
		{		
			...
			if (lpDrawItemStruct->itemState&ODS_FOCUS) pDC->SetTextColor(RGB(255,255,255));
			else if ( 0 < pIUserItem->GetUserData()->cbMemberOrder && 10 != pIUserItem->GetUserData()->cbMemberOrder && DTP_GAME_ID== m_wDataDescribe[i]) pDC->SetTextColor(RGB(255,0,0));
			else if(10 == pIUserItem->GetUserData()->cbMemberOrder && DTP_GAME_ID== m_wDataDescribe[i]) pDC->SetTextColor(RGB(153,50,205));
			else pDC->SetTextColor(RGB(0,0,0));
