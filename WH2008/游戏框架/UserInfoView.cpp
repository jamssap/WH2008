...
VOID CUserInfoView::OnPaint()
{
	...
	BufferDC.SetTextColor(RGB(0,64,128));
	...
	m_ImageUserListTL.BitBlt(BufferDC.GetSafeHdc(),0,0);
	for (int nXPos=m_ImageUserListTL.GetWidth(); nXPos<rcClient.Width(); nXPos+=m_ImageUserListTM.GetWidth())
	{
		m_ImageUserListTM.BitBlt(BufferDC.GetSafeHdc(),nXPos,0);
	}
	//m_ImageUserListTR.BitBlt(BufferDC.GetSafeHdc(),rcClient.Width()-m_ImageUserListTR.GetWidth(),0);
