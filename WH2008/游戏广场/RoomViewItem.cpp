...
#define IDM_UM_COPY_ID				WM_USER+102							//拷贝 I D
#define	IDM_UM_SET_CHAT				WM_USER+103							//设置聊天
#define	IDM_UM_CANECL_CHAT			WM_USER+104							//取消聊天
#define	IDM_UM_LOOK_LOCATION		WM_USER+105							//查看位置
#define	IDM_UM_INVITE_PLAY			WM_USER+106							//邀请游戏
#define	IDM_UM_SEARCH_USER			WM_USER+107							//寻找用户
#define	IDM_UM_SET_FRIEND			WM_USER+108							//设为好友
#define	IDM_UM_SET_DETEST			WM_USER+109							//设为厌恶
...
BEGIN_MESSAGE_MAP(CRoomViewItem, CDialog)
	...
	ON_BN_CLICKED(IDC_CLEAR,	OnBnClickedClearScore)
...
CRoomViewItem::CRoomViewItem() : CDialog(IDD_GAME_ROOM), 
	m_VorSplitter(VorSpliter), 
	m_HorSplitter(HorSpliter),
	m_BankStorage(0)
{
	//辅助变量
	m_wGameGenre=0;
	...
	m_ImageNoteL.SetLoadInfo(IDB_NOTE_L,hInstance,false);
	m_ImageNoteM.SetLoadInfo(IDB_NOTE_M,hInstance,false);
	m_ImageNoteR.SetLoadInfo(IDB_NOTE_R,hInstance,false);
...
void CRoomViewItem::DoDataExchange(CDataExchange * pDX)
{
	...
	DDX_Control(pDX, IDC_CLEAR,m_btClearScore);
...
BOOL CRoomViewItem::OnInitDialog()
{
	...
	m_ChatMessage.SetBackgroundColor(FALSE,RGB(225,240,254));

	//创建控件
	m_BrowerAD.Create(NULL,NULL,WS_VISIBLE|WS_CHILD,CRect(0,0,0,0),this,100,NULL);
	m_BrowerAD.Navigate(TEXT("http://www.52e8.com/AD/GameRoomAD.asp"),NULL,NULL,NULL,NULL);

	/*m_BrowerNote.Create(NULL,NULL,WS_VISIBLE|WS_CHILD,CRect(0,0,0,0),this,101,NULL);
	m_BrowerNote.Navigate(TEXT("http://www.52e8.com/AD/GameRoomAD.asp"),NULL,NULL,NULL,NULL);*/
	...
	m_ToolTipCtrl.AddTool(GetDlgItem(IDC_BUGLE),TEXT("发送小喇叭,本房间所有人可以看到"));
	...
	case IDM_UM_COPY_ID:
		{
			...
			LPTSTR szString=(LPTSTR)GlobalLock(hData);
			sprintf(szString, TEXT("%ld"), pIUserItem->GetUserData()->dwGameID);
...
void CRoomViewItem::OnBnClickedBank()
{
	...
	m_BankStorage.SetSendInfo(NULL,m_ClientSocket.GetInterface(),pMeUserData);
...
//负分清零
void CRoomViewItem::OnBnClickedClearScore()
{
	//类型判断	
	...
	PropertyHeadInfo.cbSendLocation=GetLocation();
	PropertyHeadInfo.nPropertyID=PROP_NEGAGIVE;
	PropertyHeadInfo.dwSourceUserID=m_pMeUserItem->GetUserID();
	PropertyHeadInfo.dwTargetUserID=m_pMeUserItem->GetUserID();
	PropertyHeadInfo.dwPachurseCount=1;
	PropertyHeadInfo.dwOnceCount=1;

	//发送数据
	m_ClientSocket->SendData(MDM_GF_PRESENT, SUB_GF_PROPERTY, &PropertyHeadInfo, sizeof(PropertyHeadInfo));
}
...
bool CRoomViewItem::ShowUserInfoMenu(IUserItem * pIUserItem, CPoint Point)
{
	...
	UserInfoMenu.AppendMenu(0,IDM_UM_COPY_ID,TEXT("拷贝用户ID"));
	...
	if (m_wGameGenre==GAME_GENRE_GOLD)
	{
		_snprintf(szBuffer,CountArray(szBuffer),TEXT("金币：%I64d  逃跑率：%5.2f%%"),pUserData->lScore,dFleeRate);
			UserInfoMenu.AppendMenu(0,0,szBuffer);
	}
	else
	{
...
bool CRoomViewItem::UpdateSkinResource()
{
	...
	m_btClearScore.SetButtonImage(GetServerViewRes().uBtClearScore,hInstance,false);
...
void CRoomViewItem::DrawLeftViewFrame(CDC * pDC)
{
	...
	//绘画背景
	CRect rcRightArea(rcVorSplitter.right,0,rcClient.right,rcClient.bottom);
	pDC->FillSolidRect(rcRightArea,RGB(25,119,137));

	//通告边框
	//CImageHandle ImageHandleNotL(&m_ImageNoteL);
	//CImageHandle ImageHandleNotM(&m_ImageNoteM);
	//CImageHandle ImageHandleNotR(&m_ImageNoteR);
	//m_ImageNoteL.BitBlt(pDC->GetSafeHdc(),rcVorSplitter.right,0);
	//for (int nXPos=rcVorSplitter.right+m_ImageNoteL.GetWidth(); nXPos<rcClient.Width()-m_ImageNoteR.GetWidth(); nXPos+=m_ImageNoteM.GetWidth())
	//{
	//	m_ImageNoteM.BitBlt(pDC->GetSafeHdc(),nXPos,0);
	//}
	//m_ImageNoteR.BitBlt(pDC->GetSafeHdc(),rcClient.Width()-m_ImageNoteR.GetWidth(),0);
	...
	rcList.top=0;
	rcList.bottom=rcList.top+m_ImageInfoList.nTBorder+120+m_ImageInfoList.nBBorder;
	CSkinAide::DrawEncircleFrame(pDC,rcList,m_EncircleList);

	//绘画列表
	rcList.left=rcVorSplitter.right;
	rcList.right=rcClient.Width();
	rcList.top=120+m_ImageInfoList.nBBorder+m_ImageInfoList.nTBorder;
	rcList.bottom=rcHorSplitter.top-m_PropertyBar.GetHeight();
	CSkinAide::DrawEncircleFrame(pDC,rcList,m_EncircleList);
...
void CRoomViewItem::RectifyControl(int nWidth, int nHeight)
{
	...
	//广告区域
	CImageHandle ImageHandleNotL(&m_ImageNoteL);
	DeferWindowPos(hDwp,m_BrowerAD,NULL,rcVorSplitter.right+m_ImageInfoList.nLBorder,m_ImageInfoList.nTBorder,nWidth-rcVorSplitter.right-m_ImageInfoList.nLBorder-m_ImageInfoList.nRBorder,120,uFlags);
	/*DeferWindowPos(hDwp,m_BrowerNote,NULL,rcVorSplitter.right+m_ImageInfoList.nLBorder,2,nWidth-rcVorSplitter.right-m_ImageInfoList.nLBorder-m_ImageInfoList.nRBorder,
		m_ImageNoteL.GetHeight()-4,uFlags);*/
	...
	{
		//列表区域
		int nControlYPos=m_ImageInfoList.nTBorder*2+120+m_ImageInfoList.nBBorder;
		int nControlHeight=rcHorSplitter.top-m_PropertyBar.GetHeight()-m_ImageInfoList.nBBorder-nControlYPos;
		DeferWindowPos(hDwp,m_UserListView,NULL,rcVorSplitter.right+m_ImageInfoList.nLBorder,nControlYPos,
			nWidth-rcVorSplitter.right-m_ImageInfoList.nLBorder-m_ImageInfoList.nRBorder,
			nControlHeight,uFlags);

		//道具区域
		m_PropertyBar.ShowWindow(SW_SHOW);
		DeferWindowPos(hDwp, m_PropertyBar, NULL, rcVorSplitter.right, rcHorSplitter.top-m_PropertyBar.GetHeight(), 
			nWidth-rcVorSplitter.right-1, 
			m_PropertyBar.GetHeight(), uFlags); 
		...
		DeferWindowPos(hDwp,m_ChatMessage,NULL,rcVorSplitter.right+m_ImageInfoChat.nLBorder,
			rcHorSplitter.top+m_ImageInfoChat.nTBorder,
			nWidth-rcVorSplitter.right-m_ImageInfoChat.nRBorder-m_ImageInfoChat.nLBorder,
			nHeight-rcHorSplitter.bottom-m_ImageInfoChat.nBBorder+rcHorSplitter.Height()/2,uFlags);
		...
		{
			DeferWindowPos(hDwp,pButtonArray[i]->m_hWnd,NULL,rcVorSplitter.right+m_ImageInfoChat.nLBorder+(rcButton.Width()+5)*i,nHeight-50,rcButton.Width(),rcButton.Height(),uFlags|SWP_NOSIZE);
		}
		m_btSendChat.GetWindowRect(&rcButton);
		DeferWindowPos(hDwp,m_btSendChat,NULL,nWidth-rcButton.Width()-8,nHeight-26,rcButton.Width(),rcButton.Height(),uFlags);
		DeferWindowPos(hDwp,m_ChatObject,NULL,rcVorSplitter.right+8,nHeight-26,70,200,uFlags);
		DeferWindowPos(hDwp,m_ChatInput,NULL,rcVorSplitter.right+81,nHeight-26,nWidth-rcVorSplitter.right-rcButton.Width()-93,19,uFlags);
	}
	...
	int nBtExcursionY=3;
	DeferWindowPos(hDwp,m_btQuitRoom,NULL,rcVorSplitter.left-rcButton.Width()-m_ImageInfoTable.nRBorder-12,nBtExcursionY,rcButton.Width(),rcButton.Height(),uFlags);
	DeferWindowPos(hDwp,m_btAutoSit,NULL,rcVorSplitter.left-rcButton.Width()*2-m_ImageInfoTable.nRBorder-20,nBtExcursionY,rcButton.Width(),rcButton.Height(),uFlags);
	DeferWindowPos(hDwp,m_btFindUser,NULL,rcVorSplitter.left-rcButton.Width()*3-m_ImageInfoTable.nRBorder-28,nBtExcursionY,rcButton.Width(),rcButton.Height(),uFlags);
	DeferWindowPos(hDwp,m_btClearScore,NULL,rcVorSplitter.left-rcButton.Width()*4-m_ImageInfoTable.nRBorder-60,nBtExcursionY,rcButton.Width(),rcButton.Height(),uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btBank,NULL,rcVorSplitter.left-rcButton.Width()*4-m_ImageInfoTable.nRBorder-120,nBtExcursionY,rcButton.Width(),rcButton.Height(),uFlags|SWP_NOSIZE);
...
bool __cdecl CRoomViewItem::OnEventTCPSocketRead(WORD wSocketID, CMD_Command Command, VOID * pData, WORD wDataSize)
{
	...
	case MDM_GF_BANK:
		{
			return OnSocketMainBank(Command,pData,wDataSize);
		}
	}
...
bool CRoomViewItem::OnSocketMainGameFrame(CMD_Command Command, void * pData, WORD wDataSize)
{
	...
	switch (Command.wSubCmdID)
	{
	case SUB_GF_RESIDUAL_PROPERTY:	//道具数目
		{
			//效验参数
	...
	case SUB_GF_PROP_ATTRIBUTE:	//道具消息
		{
			//校验参数
...
//银行消息
bool CRoomViewItem::OnSocketMainBank(CMD_Command Command, void * pBuffer, WORD wDataSize)
{
	ASSERT(Command.wMainCmdID==MDM_GF_BANK);
	switch (Command.wSubCmdID)
	{
	case SUB_GF_BANK_RESULT:
		{
			ASSERT(wDataSize==sizeof(CMD_GF_BankResult));
			if (wDataSize!=sizeof(CMD_GF_BankResult)) return false;

			CMD_GF_BankResult *pBankResult=(CMD_GF_BankResult *)pBuffer;

			if (pBankResult->cbGameAction) //游戏中存取
			{
				IPC_BankResult BankResult;
				CopyMemory(&BankResult,pBankResult,sizeof(BankResult));
				SendProcessData(IPC_MAIN_BANK,IPC_SUB_BANK_RESULT,&BankResult,sizeof(BankResult));
			}
			else //房间中存取
			{
				if (pBankResult->szErrorDescribe[0]!=0) 
					ShowInformationEx(pBankResult->szErrorDescribe,0,MB_ICONINFORMATION,TEXT("温馨提示"));
				else if(pBankResult->lResultType==3) 
					ShowInformationEx(TEXT("转帐成功：金币已经成功转出/增送，请告知玩家/朋友及时查收！"),0,MB_ICONINFORMATION,TEXT("温馨提示"));
				else 
					ShowInformationEx(TEXT("本次银行操作成功！"),0,MB_ICONINFORMATION,TEXT("温馨提示"));
				if (IsWindow(m_BankStorage)) m_BankStorage.SetWaiting(false);
			}
...
bool CRoomViewItem::OnSocketSubSitFailed(CMD_Command Command, void * pData, WORD wDataSize)
{
	...
	CString strMessage(pSitFailed->szFailedDescribe);
	if (-1!=strMessage.Find(TEXT("加入游戏至少需要")) && -1!=strMessage.Find(TEXT("不能加入")))
		ShellExecute(NULL,TEXT("open"),"http://www.52e8.com/Pay.asp",NULL,NULL,SW_SHOWDEFAULT);
	...
	case IPC_SUB_IPC_CLIENT_CONNECT:	//连接消息
		{
			...
			if ((m_wPacketTableID!=INVALID_TABLE)&&(m_wPacketChairID!=INVALID_CHAIR))
			{
				...
				do
				{
					...
					TCHAR szMsgType[128];
					_sntprintf(szMsgType,CountArray(szMsgType),TEXT("main:%d,sub:%d"),SocketPackage.Command.wMainCmdID,SocketPackage.Command.wSubCmdID);
                    m_MessageProxyHelper->InsertSystemString(szMsgType,0);
