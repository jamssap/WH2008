...
BOOL CAutoUpdateDlg::OnInitDialog()
{
	...
	hNotifyWindow = ::FindWindow(_T("mhGamePlaza"), NULL);
//#ifndef _DEBUG
...
//#endif
	...
	//读取配置参数
	strSiteURL = AfxGetApp()->GetProfileString(_T("Download"), _T("site1"), _T("http://www.86973.com"));
	...
	//	ASSERT(hNotifyWindow!=NULL);
		...
		//AfxMessageBox(strSiteURL);
		strcat(szModuleName,_T("\\mhgame.exe "));
		strcat(szModuleName,"noupdate");
		
		WinExec(szModuleName, SW_SHOWDEFAULT);
	


            //strcat(szModuleName, _T("\\GamePlaza.exe 7611074 2d3ee793eabb8e5a68b0e9d2cf8c91c2 0 update"));
...
void CAutoUpdateDlg::OnStatusCallback(ULONG ulProgress, ULONG ulProgressMax, ULONG ulStatusCode, LPCTSTR pszStatusText)
{
	...
	case DMS_ENDSETUP:
		{
			...
			//strcat(szModuleName, _T("\\GamePlaza.exe wdnynmp 1bbd886460827015e5d605ed44252251"));		    
			strcat(szModuleName, _T("\\mhgame.exe "));//要加一个空格
			strcat(szModuleName,"noupdate");
			WinExec(szModuleName, SW_SHOWDEFAULT);
	...
	case DMS_ERROR:
		{
			CString strError = AfxGetApp()->GetProfileString(_T("messages"), _T("ErrorMsg"), _T("更新失败，请到http://www.86973.com/下载最新版本"));
