...
bool __cdecl CDataBase::SetConnectionInfo(LPCTSTR szIP, WORD wPort, LPCTSTR szData, LPCTSTR szName, LPCTSTR szPass)
{
	...
	//m_strConnect.Format(TEXT("Password=%s;Persist Security Info=True;User ID=%s;Initial Catalog=%s;Data Source=%s,%ld;"),
	//	szPass,szName,szData,szIP,wPort);
	return true;
}
...
LPCTSTR CDataBase::GetComErrorDescribe(CComError & ComError)
{
	...
	m_strErrorDescribe.Format(TEXT("ADO 错误：0x%8x，%s,%s"),ComError.Error(),(LPCTSTR)bstrDescribe,m_strConnect);
