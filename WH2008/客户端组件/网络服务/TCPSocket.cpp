...
DWORD __cdecl CTCPSocket::Connect(DWORD dwServerIP, WORD wPort)
{
 ...
 //   	CString temp;
	...
	if (dwServerIP==INADDR_NONE) 
	{
          ...
          //dwServerIP=TEXT("122.228.217.66");
		//throw TEXT("目标服务器地址格式不正确，请检查后再次尝试！");

  //     		LPCSTR pszServerAddr="122.228.217.66";
...
LRESULT CTCPSocket::OnSocketNotifyRead(WPARAM wParam, LPARAM lParam)
{
	try
	{
		...
		while (m_wRecvSize>=sizeof(CMD_Head))
		{
			...
			//处理数据
			bool bSuccess=m_pITCPSocketSink->OnEventTCPSocketRead(m_wSocketID,Command,pDataBuffer,wDataSize);//登录验证 wdnynmp
