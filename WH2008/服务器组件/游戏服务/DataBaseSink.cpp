...
bool __cdecl CDataBaseSink::OnDataBaseEngineRequest(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBR_GR_BANK_TRANSFER:			//转出金币
		{
...
bool CDataBaseSink::OnBankDrawoutGold(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	{
		//提取金币
		lReturnValue = SPBankDrawoutGold(pBankGet->dwUserID,pBankGet->DrawoutGoldCount,
			pBankGet->lCurrentScore,pBankGet->lCurrentBankScore,pBankGet->dwClientIP);
	}
	catch (IDataBaseException * pIException)
	{
		//错误信息
...
bool CDataBaseSink::OnBankStorageGold(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	{
		//提取金币
		lReturnValue = SPBankStorageGold(pBankStorage->dwUserID,pBankStorage->lStorageCount,
			pBankStorage->lCurrentScore,pBankStorage->lCurrentBankScore,pBankStorage->dwClientIP);
	}
	catch (IDataBaseException * pIException)
	{
		//错误信息
...
//提取存储过程
LONG CDataBaseSink::SPBankDrawoutGold(DWORD dwUserID, LONGLONG lSwapScore, LONGLONG lCurrentScore, LONGLONG lCurrentBankScore, DWORD dwClientIP)
{
	ASSERT( dwUserID != 0 );
...
//存金存储过程
LONG CDataBaseSink::SPBankStorageGold(DWORD dwUserID, LONGLONG lSwapScore, LONGLONG lCurrentScore, LONGLONG lCurrentBankScore, DWORD dwClientIP)
{
	ASSERT( dwUserID != 0 );
									   ...
									   LONGLONG lCurrentScore, LONGLONG lCurrentBankScore, BYTE cbUserOnline, DWORD dwClientIP)
{
	...
	m_GameScoreDBAide.AddParameter(TEXT("@lCurrentScore"),lCurrentScore);
	m_GameScoreDBAide.AddParameter(TEXT("@lCurrentBankScore"),lCurrentBankScore);
	m_GameScoreDBAide.AddParameter(TEXT("@UserOnline"),cbUserOnline);
	m_GameScoreDBAide.AddParameter(TEXT("@dwKindID"),m_pGameServiceAttrib->wKindID);
	m_GameScoreDBAide.AddParameter(TEXT("@dwServerID"),m_pGameServiceOption->wServerID);
	m_GameScoreDBAide.AddParameter(TEXT("@strClientIP"),szClientIP);
...
LONG CDataBaseSink::SPBankSecurity(DWORD dwUserID, LPCTSTR pszPassword, DWORD dwClientIP)
{
	...
	m_AccountsDBAide.AddParameter(TEXT("@dwKindID"),m_pGameServiceAttrib->wKindID);
