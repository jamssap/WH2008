...
const int			CGameFrameView::m_nXBorder=8;						//定时器高
const int			CGameFrameView::m_nYBorder=10;						//定时器高
...
void CGameFrameView::DrawViewImage(CDC * pDC, CSkinImage & SkinImage, enImageMode ImageMode)
{
	...
	//边框资源
	CImageHandle ImageHandleFrameLB(&m_ImageFrameLB);
	CImageHandle ImageHandleFrameLM(&m_ImageFrameLM);
	CImageHandle ImageHandleFrameLT(&m_ImageFrameLT);
	CImageHandle ImageHandleFrameBL(&m_ImageFrameBL);
	CImageHandle ImageHandleFrameBM(&m_ImageFrameBM);
	CImageHandle ImageHandleFrameBR(&m_ImageFrameBR);
	CImageHandle ImageHandleFrameTM(&m_ImageFrameTM);
	CImageHandle ImageHandleFrameMM(&m_ImageFrameMM);

	//绘画边框
	for (int nYPos=0; nYPos<rcClient.bottom; nYPos+=m_ImageFrameMM.GetHeight()) m_ImageFrameMM.BitBlt(pDC->GetSafeHdc(),rcClient.right-m_ImageFrameMM.GetWidth(),nYPos);
	for (int nXPos=0; nXPos<rcClient.right; nXPos+=m_ImageFrameTM.GetWidth()) m_ImageFrameTM.BitBlt(pDC->GetSafeHdc(),nXPos,0);
	m_ImageFrameLT.BitBlt(pDC->GetSafeHdc(),0,0);
	for (int nYPos=m_ImageFrameLT.GetHeight(); nYPos<rcClient.bottom-m_ImageFrameLB.GetHeight(); nYPos+=m_ImageFrameLM.GetHeight())
		m_ImageFrameLM.BitBlt(pDC->GetSafeHdc(),0,nYPos);
	m_ImageFrameLB.BitBlt(pDC->GetSafeHdc(),0,rcClient.bottom-m_ImageFrameLB.GetHeight());

	m_ImageFrameBL.BitBlt(pDC->GetSafeHdc(),0,rcClient.bottom-m_ImageFrameBL.GetHeight());
	for (int nXPos=m_ImageFrameBL.GetWidth(); nXPos<rcClient.right; nXPos+=m_ImageFrameBM.GetWidth())
		m_ImageFrameBM.BitBlt(pDC->GetSafeHdc(),nXPos,rcClient.bottom-m_ImageFrameBL.GetHeight());
	//m_ImageFrameBR.BitBlt(pDC->GetSafeHdc(),rcClient.right-m_ImageFrameBR.GetWidth(),rcClient.bottom-m_ImageFrameBL.GetHeight());
