	...
	else
	{
		...
		if (lLessScore!=0L || lMaxScore>lLessScore)
		{
			if (pUserData->UserScoreInfo.lScore<lLessScore)
			{
				...
				{
					_snprintf(szDescribe,sizeof(szDescribe),TEXT("加入游戏至少需要 %I64d 个金币，您的金币不够，不能加入！"),lLessScore);
				}
				else
				{
...
bool __cdecl CTableFrame::ConcludeGame()
{
		...
		for (WORD i=0;i<m_wChairCount;i++)
		{
			if (m_pIUserItem[i]!=NULL)
			{
				...
				if (pUserData->UserScoreInfo.lScore<lLessScore)
				{
					...
					if (m_pGameServiceOption->wServerType==GAME_GENRE_GOLD)
					{
						_snprintf(szDescribe,sizeof(szDescribe),TEXT("您的金币少于 %I64d，不能继续游戏！"),lLessScore);
					}
					else 
					{
						_snprintf(szDescribe,sizeof(szDescribe),TEXT("您的游戏积分少于 %I64d，不能继续游戏！"),lLessScore);
					}
					SendGameMessage(pIServerUserItem,szDescribe,SMT_EJECT|SMT_INFO|SMT_CLOSE_GAME);
	...
	////状态设置
	//if (m_pGameServiceAttrib->cbJoinInGame==TRUE)
	//{
	//	WORD wNullChairArray[MAX_CHAIR];
	//	WORD wNullCount=0;

	//	//找出空位子
	//	for (WORD i=0; i < m_pGameServiceAttrib->wChairCount; i++)
	//	{
	//		if (m_pIUserItem[i]==NULL) wNullChairArray[wNullCount++]=i;
	//	}


	//	//修改旁观状态
	//	INT_PTR nCount=__min(m_LookonUserItemPtr.GetCount(),wNullCount);
	//	for (INT_PTR i=0; i < nCount; i++)
	//	{
	//		IServerUserItem *pIServerUserItem = m_LookonUserItemPtr.GetAt(i);
	//		ASSERT(pIServerUserItem!=NULL);
	//		pIServerUserItem->SetUserStatus(US_FREE,INVALID_TABLE,INVALID_CHAIR);
	//		m_pIGameServiceFrame->SendUserStatus(pIServerUserItem);

	//		ASSERT(m_pIUserItem[wNullChairArray[i]]==NULL);
	//		m_pIUserItem[wNullChairArray[i]]=pIServerUserItem;
	//		pIServerUserItem->SetUserStatus(US_SIT,m_wTableID,wNullChairArray[i]);
	//	}
	//	m_LookonUserItemPtr.RemoveAt(0,nCount);
	//}
...
//写入积分
bool __cdecl CTableFrame::WriteUserScore(WORD wChairID, LONGLONG lScore, LONG lRevenue, enScoreKind ScoreKind, LONG lPlayTimeCount)
{
	//效验参数
	ASSERT(wChairID<m_wChairCount);
	ASSERT(m_pIUserItem[wChairID]!=NULL);
	...
	if (m_pIUserItem[wChairID]==NULL) 
	{
		OUTLOG(TEXT("写分失败，写入分数的目标用户已离开，写入分数：%I64d"),lScore);
		return false;
	}
...
//写入积分
bool __cdecl CTableFrame::WriteTableScore(tagScoreInfo ScoreInfoArray[], WORD wScoreCount, LONG lRevenue)
{
	ASSERT(m_pIGameServiceFrame!=NULL);

	//写分统计
	LONGLONG lWinScore=0;
	LONGLONG lLostScore=0;
	int nLostCount=0,nWinCount=0,nDrawCount=0;

	for (WORD i=0; i<wScoreCount;i++)
	{
		if (ScoreInfoArray[i].lScore>0)
		{
			lWinScore+=ScoreInfoArray[i].lScore;
			nWinCount++;
		}
		else if (ScoreInfoArray[i].lScore<0)
		{
			lLostScore+=ScoreInfoArray[i].lScore;
			nLostCount++;
		}
		else
		{
	...
	//求绝对值
	lLostScore = lLostScore < 0 ? -lLostScore : lLostScore;
	if (lWinScore>lLostScore)
	{
		OUTLOG(TEXT("写分异常，统计输赢得分不一致，赢的分必须小于等于输的分，统计结果赢：%I64d 输：%I64d，输家人数：%d，赢家人数：%d，平家人数：%d"), 
			lWinScore,-lLostScore,nLostCount,nWinCount,nDrawCount);
	}
	...
	for (WORD i=0; i < wScoreCount; i++)
	{
		if (ScoreInfoArray[i].lScore==0) continue;
		IServerUserItem *pServerUserItem=m_pIUserItem[i];

		//写分失败
		if (pServerUserItem==NULL)
		{
			OUTLOG(TEXT("写分失败，写入分数的目标用户已离开，写入分数：%I64d"),ScoreInfoArray[i].lScore);
			continue;
		}

		WriteUserScore(pServerUserItem,ScoreInfoArray[i].lScore,ScoreInfoArray[i].lRevenue,ScoreInfoArray[i].ScoreKind);
	}
...
//写入积分
bool __cdecl CTableFrame::WriteUserScore(IServerUserItem * pIServerUserItem, LONGLONG lScore, LONG lRevenue, enScoreKind ScoreKind, LONG lPlayTimeCount)
{
	//效验参数
	ASSERT(pIServerUserItem!=NULL);
