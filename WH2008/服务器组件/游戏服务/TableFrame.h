...
public:
	//写入积分
	virtual bool __cdecl WriteUserScore(WORD wChairID, LONGLONG lScore, LONG lRevenue, enScoreKind ScoreKind, LONG lPlayTimeCount=-1);
	//写入积分
	virtual bool __cdecl WriteUserScore(IServerUserItem * pIServerUserItem, LONGLONG lScore, LONG lRevenue, enScoreKind ScoreKind, LONG lPlayTimeCount=-1);
	//写入积分
	virtual bool __cdecl WriteTableScore(tagScoreInfo ScoreInfoArray[], WORD wScoreCount, LONG lRevenue);
