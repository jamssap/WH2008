...
void CPropertyItem::UpdateView()
{
	...
	m_strNormalUserPrice.Format(TEXT("普通用户需支付金币：%ld"), lNormalPrice);
	m_strMemberUserPrice.Format(TEXT("会员用户需支付金币：%ld"), lMemberPrice);	
...
void CFlowerSetting::UpdateView()
{
	...
	LONGLONG lNormalPrice = CPropertyBar::m_FlowerInfo[m_nFlowerID].lPrice * m_nFlowerCount;
	LONGLONG lMemberPrice = LONG(lNormalPrice * (CPropertyBar::m_FlowerInfo[m_nFlowerID].cbDiscount / 100.));
	m_strNormalUserPrice.Format(TEXT("普通用户需支付金币：%I64d"), lNormalPrice);
	m_strMemberUserPrice.Format(TEXT("会员用户需支付金币：%I64d"), lMemberPrice);	
