...
//构造函数
CClientKernel::CClientKernel() : m_BankStorage(1)
{
	//房间信息
	m_dwUserID=0L;
...
//显示银行
bool __cdecl CClientKernel::ShowBankDialog(UINT nSelPage,DWORD dwOption)
{
	if (m_pMeUserItem==NULL) return false;

	//设置信息
	m_BankStorage.SetSendInfo(this,NULL,m_pMeUserItem);

	//创建窗口
	if (!IsWindow(m_BankStorage))
	{
		m_BankStorage.CreateBankWindow(AfxGetMainWnd(),nSelPage,dwOption);
		m_BankStorage.CenterWindow(AfxGetMainWnd());
...
bool __cdecl CClientKernel::OnChannelMessage(const IPC_Head * pHead, const void * pIPCBuffer, WORD wDataSize, HWND hWndSend)
{
	...
	case IPC_MAIN_BANK:		//银行消息
		{
			return OnIPCBank(pHead,pIPCBuffer,wDataSize,hWndSend);
		}
	}
	...
	case IPC_SUB_SERVER_INFO:	//房间信息
		{
			...
			g_GlobalOption.m_wGameGenre=pServerInfo->wGameGenre;
...
//银行消息
bool CClientKernel::OnIPCBank(const IPC_Head * pHead, const void * pIPCBuffer, WORD wDataSize, HWND hWndSend)
{
	ASSERT(pHead->wMainCmdID==IPC_MAIN_BANK);
	switch(pHead->wSubCmdID)
	{
	case IPC_SUB_BANK_RESULT:
		{
			IPC_BankResult *pBankResult=(IPC_BankResult *)pIPCBuffer;
			ASSERT(pBankResult->cbGameAction!=0);

			if (pBankResult->szErrorDescribe[0]!=0) ShowInformationEx(pBankResult->szErrorDescribe,0,MB_ICONINFORMATION,TEXT("温馨提示"));
...
void CClientKernel::UpdateGameTitle()
{
	...
	{
		CString strText;
		strText.Format(TEXT("%s ［用户名：%s］"), szTitle, m_pMeUserItem->szName);
