...
void CTabCtrlLogon::OnPaint()
{
	...
	dc.FillSolidRect(&ClipRect,RGB(244,244,244));
	...
	//CRect rcSpace(ClipRect.left,ClipRect.top,ClipRect.right,rcItem.bottom);
	//dc.FillSolidRect(rcSpace,RGB(255,255,255));
	...
	for (int i=0;i<GetItemCount();i++)
	{
		...
		dc.FillSolidRect(&rcItem,RGB(240,245,248));
...
BOOL CDlgRegister::OnInitDialog()
{
	...
	m_LineRegWeb.SetHyperLinkUrl(TEXT("http://www.52e8.com"));
	m_LineMainPage.SetHyperLinkUrl(TEXT("http://www.52e8.com"));
	m_LinePassWord.SetHyperLinkUrl(TEXT("http://www.52e8.com"));
...
BOOL CDlgLogon::OnInitDialog()
{
	...
	m_BrowerAD.Navigate(TEXT("http://www.52e8.com/AD/AD.asp"),NULL,NULL,NULL,NULL);

	//广告位置
	int nXExcursion=GetXExcursionPos();
	int nYExcursion=GetYExcursionPos();
	m_BrowerAD.MoveWindow(nXExcursion+4,nYExcursion+4,428,85);
	...
	//设置控件
	m_LineMainPage.SetHyperLinkUrl(TEXT("http://www.52e8.com/Index.asp"));
	m_LinePassWord.SetHyperLinkUrl(TEXT("http://www.52e8.com/uApplyPwd.asp"));
	m_LineGetPassWord.SetHyperLinkUrl(TEXT("http://www.52e8.com/FindPwd.asp"));
	...
	//创建刷子
	if (NULL==m_brControlBackground.m_hObject) m_brControlBackground.CreateSolidBrush(RGB(240,245,248));

	//设置颜色
	m_LineMainPage.SetBackGroundColor(RGB(240,245,248));
	m_LinePassWord.SetBackGroundColor(RGB(240,245,248));
	m_LineGetPassWord.SetBackGroundColor(RGB(240,245,248));
...
void CDlgLogon::LoadLogonServer()
{
    ...
    */
	...
	//{
	//	pComBoxServer->AddString(TEXT("122.228.217.66"));
	//	pComBoxServer->SetCurSel(0);
	//}
	if ((pComBoxServer->GetWindowTextLength()==0)&&(pComBoxServer->GetCount()>0)) pComBoxServer->SetCurSel(0);
	if (pComBoxServer->GetCount()==0)
	{
		pComBoxServer->AddString(TEXT("电信线路"));
		pComBoxServer->AddString(TEXT("网通线路"));
		...
		else
		{
		  ...
		  pComBoxServer->SetCurSel(1);
		}
		
	}

#ifdef _DEBUG
	int nSel=pComBoxServer->AddString(TEXT("127.0.0.1"));
	pComBoxServer->SetCurSel(nSel);
#endif
...
bool CDlgLogon::CheckLogonInput(bool bShowError)
{
	...
	try
	{
		...
		TCHAR szFileName[MAX_PATH]=TEXT("");
		TCHAR szTempIP[MAX_PATH]=TEXT("");
        _sntprintf(szFileName,sizeof(szFileName),TEXT("%s\\server.ini"),szModuleName);


		CString m_LoginIP="电信线路";
		CComboBox * pComBoxServer=(CComboBox *)GetDlgItem(IDC_SERVER);
		pComBoxServer->GetWindowText(m_LoginIP);



		GetPrivateProfileString(TEXT("ServerIP"),m_LoginIP,TEXT("122.228.217.66"),szTempIP,sizeof(szTempIP),szFileName);
		//GetPrivateProfileString(TEXT("ServerIP"),m_LoginIP,TEXT("www.52e8.com"),szTempIP,sizeof(szTempIP),szFileName);

		m_strLogonServer.Format("%s",szTempIP);
       
		//AfxMessageBox(m_strLogonServer);
		//登录服务器
...
void CDlgLogon::OnRegisterAccounts()
{
	...
	//ShellExecute(NULL,TEXT("open"),TEXT("http://www.52e8.com/Reg.aspx"),NULL,NULL,SW_SHOWDEFAULT);
	//return;
/*
#ifndef _DEBUG
	ShellExecute(NULL,TEXT("open"),TEXT("http://www.52e8.com/Register.asp"),NULL,NULL,SW_SHOWDEFAULT);
	return;
#endif
...
*/
	...
	//服务器
	GetDlgItemText(IDC_SERVER,m_strLogonServer);
	if (m_strLogonServer.IsEmpty()) m_strLogonServer=TEXT("122.228.217.66");
	//if (m_strLogonServer.IsEmpty()) m_strLogonServer=TEXT("www.52e8.com");
...
HBRUSH CDlgLogon::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	if (pWnd->GetSafeHwnd()==GetDlgItem(IDC_STATIC1)->GetSafeHwnd() ||
		pWnd->GetSafeHwnd()==GetDlgItem(IDC_STATIC2)->GetSafeHwnd() ||
		pWnd->GetSafeHwnd()==GetDlgItem(IDC_STATIC3)->GetSafeHwnd() ||
		pWnd->GetSafeHwnd()==GetDlgItem(IDC_STATIC4)->GetSafeHwnd() ||
		pWnd->GetSafeHwnd()==GetDlgItem(IDC_STATIC5)->GetSafeHwnd() )
	{
		//调用父类
