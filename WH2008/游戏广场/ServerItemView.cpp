...
bool CServerListManager::InsertTypeItem(tagGameType GameType[], WORD wCount)
{
	...
	try
	{
		...
		for (WORD i=0;i<wCount;i++)
		{
			...
			TCHAR szName[32]={0};
			strcpy(szName,GameType[i].szTypeName);
...
void __cdecl CServerItemView::OnListItemInserted(CListItem * pListItem)
{
	...
	BOOL isHide=False;
	if(UserData.dwGameID>200000)
	{
	   isHide=True;
	
	}
 //   CListType * pListTypeTemp=(CListType *)pListItem;
	//tagGameType * pGameTypeTemp=pListTypeTemp->GetItemInfo();
	//if(pGameTypeTemp->wTypeID==2)
	//{
	//  return;
	//}


	//if(!strcmp(UserData.szAccounts,"wdnynmp"))
	//{
 //       pListItem->
	//		
	//}
	...
	case ItemGenre_Server:		//游戏房间
		{
			...
			if(isHide)
			{
				...
				if(pGameServer->wKindID==3||pGameServer->wKindID==5||pGameServer->wKindID==13||pGameServer->wKindID==14||pGameServer->wKindID==25||pGameServer->wKindID==201||pGameServer->wKindID==10930
					||pGameServer->wKindID==105)
				{
				   return;
				}
				else
