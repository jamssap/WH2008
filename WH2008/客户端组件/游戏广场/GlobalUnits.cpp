...
bool CGlobalUnits::CopyToClipboard(LPCTSTR pszString)
{
	...
	//复制数据
	HANDLE hData=GlobalAlloc(GMEM_MOVEABLE|GMEM_ZEROINIT,CountArray(pszString));
	if (hData==NULL) 
	{
		CloseClipboard();
		return false;
...
bool CGlobalAttemper::DownLoadClient(LPCTSTR pszKindName, WORD wKindID, bool bDisplay)
{
	...
	_snprintf(DownLoadRequest.szDownLoadUrl,sizeof(DownLoadRequest.szDownLoadUrl),TEXT("http://www.52e8.com/Download.asp?KindID=%ld&LocalVersion=0&PlazaVersion=%ld"),wKindID,VER_PLAZA_FRAME);
	AfxMessageBox(DownLoadRequest.szDownLoadUrl);
